...
public class AllShardsDeallocator extends AbstractDeallocator implements ClusterStateListener {
    ...
    public ListenableFuture<DeallocationResult> deallocate() {
        RoutingNode node = clusterService.state().getRoutingNodes().node(localNodeId());
        if (isDeallocating()) {
    ...
    /**
     * can deallocate if: we have one spare node which does not contain a replica or primary of any index
     * which has shards on this node, so we can move the shards on this node to it.
     * <p>
     * More technically: number of data nodes > (maximum number_of_replicas of indices with shards on this
     * node + 1)
     */
    @Override
    public boolean canDeallocate() {
        ...
        int maxReplicas = -1;
        RoutingNode localNode = clusterState.getRoutingNodes().node(localNodeId());
        for (ObjectObjectCursor<String, IndexMetaData> entry : clusterState.metaData().indices()) {
            if (!localNode.shardsWithState(entry.key, ShardRoutingState.STARTED, ShardRoutingState.INITIALIZING, ShardRoutingState.RELOCATING).isEmpty()) {
                maxReplicas = Math.max(maxReplicas, entry.value.getNumberOfReplicas());
            }
        }
    ...
    /**
     * <ul> <li>remove exclusion for previously excluded nodeId that has been removed from the cluster
     * <li>wait for excluded nodes to have all their shards moved </ul>
     */
    @Override
    public void clusterChanged(ClusterChangedEvent event) {
        // apply new settings
