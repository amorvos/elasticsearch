...
public class AllShardsDeallocatorIT extends AbstractDeallocatorTestCase {
    ...
    public void testDeallocate() throws Exception {
        ...
        assertThat(
                ((InternalTestCluster)cluster()).getInstance(ClusterService.class, takeDownNode.name()).state().getRoutingNodes().node(takeDownNode.id()).size(),
                is(0));
        ...
        awaitBusy(new Predicate<Void>() {
            ...
            public boolean apply(Void aVoid) {
                ...
                return newState.metaData().settings().get(EnableAllocationDecider.CLUSTER_ROUTING_ALLOCATION_ENABLE, "").equals(EnableAllocationDecider.Allocation.NONE.name()) &&
                    newState.getRoutingNodes().node(takeDownNode.id()).size() == 0;
            }
    ...
    public void testDeallocateFailCannotMoveShards() throws Exception {
        client().admin().indices()
                ...
                .setSettings(Settings.builder().put("number_of_shards", 2).put("number_of_replicas", 2))
                .execute().actionGet();
        ensureGreen();
        ...
        try {
            ...
            assertThat(clusterService().state().getRoutingNodes().node(takeDownNode.id()).size(), is(2));
        }
    }
