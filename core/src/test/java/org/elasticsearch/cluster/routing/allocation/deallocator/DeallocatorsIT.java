...
public class DeallocatorsIT extends ESIntegTestCase{
    ...
    public void prepare() {
        ...
        client().admin().cluster().prepareUpdateSettings().setTransientSettings(
                Settings.builder().put(Deallocators.GRACEFUL_STOP_MIN_AVAILABILITY, "full")
        ).execute().actionGet();
    }
    ...
    public void testDeallocatorsDeallocateOngoing() throws Exception {
        ...
        // change setting
        client().admin().cluster().prepareUpdateSettings().setTransientSettings(
                Settings.builder().put(Deallocators.GRACEFUL_STOP_MIN_AVAILABILITY, "none")
        ).execute().actionGet();
        ...
        // change setting
        client().admin().cluster().prepareUpdateSettings().setTransientSettings(
                Settings.builder().put(Deallocators.GRACEFUL_STOP_MIN_AVAILABILITY, "primaries")
        ).execute().actionGet();
